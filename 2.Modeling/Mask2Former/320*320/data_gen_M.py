...
CLASS_LABELS = { 0: "Background", 1: "PV_normal", 2: "PV_heater", 3: "PV_pool", }
...
class PVDatasetFoldersCT(BaseDataset):
    ...
    def __getitem__(self, i: int):
        """
        Returns:
            image: float32 [3,H,W] already normalized by Albumentations if Normalize used
            mask : int64  [H,W] in {0,1,2,3} or 255 (ignore)
        If a sample is broken (missing/corrupt), we skip to next index.
        """
        image_path = self.images_fps[i]
        mask_path  = self.masks_fps[i]
        if not mask_path or not os.path.isfile(mask_path):
            # Skip sample gracefully
            if self.debug:
        ...
        try:
            ...
            # Albumentations expects HWC image (uint8/float) and 2D mask
            if self.augmentation is not None:
                sample = self.augmentation(image=image, mask=mask)
                image, mask = sample["image"], sample["mask"]

            # Final safety: sanitize mask values
